...
| 5.4 ◐ | **Demo recording** — `docs/demo.md` is the storyboard in three acts (a call being decided, the gate, the evidence), quoting real output from runs made to produce it. The recording asset itself is pending a format decision: no asciinema, agg, vhs, termtosvg, or ffmpeg is installed on this machine; Homebrew and uv are available, but installing tooling is the owner's call | `docs/demo.md`, recording | 5.1 |
