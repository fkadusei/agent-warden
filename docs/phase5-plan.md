...
| 5.6 ✅ | **Close-out** — `docs/not-done.md` lists every gap: key rotation and revocation (the largest), shallow inspection by design, no public timestamp authority, synthetic tools, one machine and one model, and the structural limits (Warden trusted at runtime, timestamps not post-quantum, a permissive policy is a permissive system). The benchmark's `Event loop is closed` noise is fixed: one event loop per mode instead of one per scenario, confirmed by a run with zero tracebacks. ADR-0012 accepted by the owner on 2026-09-16 | `docs/not-done.md`, `agent/bench.py` | — |
