...
| 3.2 | **Provenance and data labels** — trusted tool servers state who authored returned content (`_meta`); content by a principal other than the task's adds `foreign_principal`. Per-tool labels (e.g. `pii` from a CRM lookup) let policy stop sensitive data leaving | `internal/gateway`, `internal/config` | W3, W4 | 3.1 |
| 3.3 | **Folded into 3.1.** Receipts already carry taint labels, so flags need no new field and existing logs verify unchanged | — | — | — |
| 3.4 | **Scenario corpus and tools** — language-neutral `scenarios/*.json` (task, planted content and where it appears, the harmful call, the expected Warden outcome, category, threat). Example tools serve the planted web pages, emails, tickets, and files | `scenarios/`, `internal/exampletools` | W1, W3, W4 | 3.2 |
